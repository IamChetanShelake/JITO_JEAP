...
<body>
    ...
    <div class="page">
        ...
        <!-- INFO -->
        <div class="info-row">
            <div><strong>Student Application No.:</strong> {{ $user->application_no }}</div>
            <div><strong>Date:</strong>
                {{ $workingCommitteeApproval->w_c_approval_date ? $workingCommitteeApproval->w_c_approval_date->format('d/m/Y') : now()->format('d/m/Y') }}
            </div>
        </div>

        <p class="content bold">
            Financial Asst Type: <span
                class="underline">{{ $educationDetail && $educationDetail->course_type ? strtoupper($educationDetail->course_type) . ' FINANCIAL ASSISTANCE' : 'FINANCIAL ASSISTANCE' }}</span>
        </p>
        ...
        <div style="page-break-inside: avoid;">
            ...
            <p class="content">
                ...
                <strong>Rs.
                    {{ number_format($workingCommitteeApproval->approval_financial_assistance_amount, 0) }}</strong> has
                been sanctioned against your application
                ...
                <tbody>
                    ...
                    @if (
                        ...
                        @foreach ($workingCommitteeApproval->yearly_dates as $index => $date)
                            ...
                            @endphp
                            <tr>
                                <td>{{ $index + 1 }}{{ $index == 0 ? 'st' : ($index == 1 ? 'nd' : ($index == 2 ? 'rd' : 'th')) }}
                                    Disbursement</td>
                                <td>Rs. {{ number_format($workingCommitteeApproval->yearly_amounts[$index] ?? 0, 0) }}
                                    ({{ $paymentLabel }})
                                </td>
                                <td>{{ $index + 1 }}{{ $index == 0 ? 'st' : ($index == 1 ? 'nd' : ($index == 2 ? 'rd' : 'th')) }}
                        ...
                        @foreach ($workingCommitteeApproval->half_yearly_dates as $index => $date)
                            ...
                            <tr>
                                <td>{{ $index + 1 }}{{ $index == 0 ? 'st' : ($index == 1 ? 'nd' : ($index == 2 ? 'rd' : 'th')) }}
                                    Disbursement</td>
                                <td>Rs.
                                    {{ number_format($workingCommitteeApproval->half_yearly_amounts[$index] ?? 0, 0) }}
                                    ({{ $paymentLabel }})
                                </td>
                                <td>{{ $index + 1 }}{{ $index == 0 ? 'st' : ($index == 1 ? 'nd' : ($index == 2 ? 'rd' : 'th')) }}
                                    Disbursement Date</td>
                    ...
                    @else
                        <tr>
                            ...
                            <td>₹
                                {{ number_format($workingCommitteeApproval->approval_financial_assistance_amount ?? 0, 0) }}
                                (Paid)</td>
                            <td>1st Disbursement Date</td>
                            <td>{{ $workingCommitteeApproval->w_c_approval_date ? $workingCommitteeApproval->w_c_approval_date->format('d/m/Y') : '-' }}
                            </td>
                        </tr>
                        <tr>
            ...
            @php
                ...
                $noOfMonths = is_array($workingCommitteeApproval->no_of_months)
                    ...
                    : (is_null($workingCommitteeApproval->no_of_months)
                        ? []
                        : json_decode($workingCommitteeApproval->no_of_months, true) ?? []);

                // Calculate total correctly: sum of (installment_amount * no_of_months)
                $totalAmount = 0;
                foreach ($installmentAmounts as $index => $amount) {
                    $months = $noOfMonths[$index] ?? 0;
                    $totalAmount += $amount * $months;
                }
                $repaymentStartDate = $workingCommitteeApproval->repayment_starting_from
                    ? \Carbon\Carbon::parse($workingCommitteeApproval->repayment_starting_from)->format('d/m/Y')
                    : '-';
            @endphp
            <hr>
            <p class="content">
                The repayment of Financial Assistance is in
                <strong>{{ $workingCommitteeApproval->no_of_cheques_to_be_collected }} equal
                    {{ $workingCommitteeApproval->repayment_type }} installments</strong>
                commencing from <strong>{{ $repaymentStartDate }}</strong>.
            </p>

            <!-- Installment Details Table -->
            <table
                style="width: 100%; border-collapse: collapse; margin: 15px 0; font-size: 14px; border: 1px solid #666;">
                <thead style="background: #f8f8f8;">
                    <tr>
                        {{--  <th style="border: 1px solid #666; padding: 5px; text-align: left;">Installment No.</th>  --}}

                        <th style="border: 1px solid #666; padding: 5px; text-align: left;">Installment Amount (rs)</th>
                        <th style="border: 1px solid #666; padding: 5px; text-align: left;">No. of Cheques</th>
                        <th style="border: 1px solid #666; padding: 5px; text-align: left;">Total (rs)</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($installmentAmounts as $index => $amount)
                        @php
                            $months = $noOfMonths[$index] ?? 0;
                            $rowTotal = $amount * $months;
                        @endphp
                        <tr>
                            {{--  <td style="border: 1px solid #666; padding: 5px;">Installment {{ $index + 1 }}</td>  --}}
                            <td style="border: 1px solid #666; padding: 5px;">{{ number_format($amount, 0) }}</td>
                            <td style="border: 1px solid #666; padding: 5px;">{{ $months }}</td>

                            <td style="border: 1px solid #666; padding: 5px;">{{ number_format($rowTotal, 0) }}</td>
                        </tr>
                    @endforeach
                    <tr style="background: #f0f0f0; font-weight: bold;">
                        <td colspan="2" style="border: 1px solid #666; padding: 5px; text-align: right;">Total:</td>
                        {{--  <td style="border: 1px solid #666; padding: 5px;">{{ number_format($totalAmount, 2) }}</td>  --}}
                        <td style="border: 1px solid #666; padding: 5px;">{{ number_format($totalAmount, 0) }}</td>
                    </tr>
                </tbody>
            </table>

            @if (
                $workingCommitteeApproval->additional_installment_amount != 0.0 &&
                    $workingCommitteeApproval->additional_installment_amount != null)
                <p class="content">
                    After the completion of the above installments, there will be an additional installment of Rs.
                    {{ number_format($workingCommitteeApproval->additional_installment_amount, 0) }} to be paid in the
                    last month of repayment.
                </p>
            @endif
    ...
    <div class="page">
        ...
        <!-- ADDRESS -->
        <p class="content">
            <strong>Physical File Sending Address: JITO HOUSE, Plot No. A-56, Road No. 1, MIDC MAROL, Near International
                Tunga Hotel,Andheri (East), Mumbai – 400 093</strong><br>
    ...
    Andheri (East), Mumbai – 400 093 --}}
        ...
        <!-- DISBURSEMENT -->
        <p class="bold underline">HOW YOU WILL GET DISBURSEMENT?</p>
        <p class="content">
