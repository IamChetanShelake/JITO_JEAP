                ...
                <div class="col-12">
                    @php
                        ...
                        $isDomesticGraduation =
                            $user->financial_asset_type === 'domestic' && $user->financial_asset_for === 'graduation';
                        $isDomesticPg =
                            $user->financial_asset_type === 'domestic' &&
                            $user->financial_asset_for === 'post_graduation';
                    ...
                    <form method="POST" action="{{ route('user.step7.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="d-flex justify-content-between mt-4 mb-4">
                            <button type="button" class="btn " style="background:#988DFF1F;color:gray;"><svg
                                ...
                                Previous</button>
                            {{-- @if ($user->submit_status == 'submited' && ($user->educationDetail && $user->educationDetail->submit_status == 'submited') && ($user->familyDetail && $user->familyDetail->submit_status == 'submited') && ($user->fundingDetail && $user->fundingDetail->submit_status == 'submited') && ($user->guarantorDetail && in_array($user->guarantorDetail->submit_status, ['submited', 'skipped'])) && ($useDocumentsBelow ? $user->documentsBelow && $user->documentsBelow->submit_status == 'submited' : ($useDocumentBelowPg ? $user->documentBelowPg && $user->documentBelowPg->submit_status == 'submited' : $user->document && $user->document->submit_status == 'submited')) && $user->application_status != 'submitted') --}}
                            @php
                                $isGuarantorValid =
                                    $isBelowOneLakh ||
