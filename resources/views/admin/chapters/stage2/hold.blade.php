...
@section('content')
    ...
    <div class="card">
        <div class="card-body" style="padding: 1.5rem; border-bottom: 1px solid var(--border-color);">
            <form method="GET" action="{{ route($filterRoute ?? 'admin.chapter.resubmit') }}" id="filterForm">
                                ...
                                <td>
                                    @php
                                        ...
                                        if (
                                            ...
                                            $user->workflowStatus &&
                                            $user->workflowStatus->apex_2_status === 'approved'
                                        ) {
                                            ...
                                            $user->workflowStatus &&
                                            $user->workflowStatus->chapter_reject_remarks
                                        ) {
                                            $status = 'Resubmitted';
                                            $statusClass = 'status-hold';
                                    ...
                                    @if ($user->workflowStatus && $user->workflowStatus->chapter_reject_remarks)
                                        <div class="hold-reason">Reason:
                                            {{ $user->workflowStatus->chapter_reject_remarks }}</div>
                                    @elseif($user->workflowStatus && $user->workflowStatus->apex_1_reject_remarks)
                                        <div class="hold-reason">Reason: {{ $user->workflowStatus->apex_1_reject_remarks }}
                                        </div>
                                    @else
