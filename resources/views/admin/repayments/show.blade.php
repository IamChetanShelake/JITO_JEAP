        ...
        <div class="detail-container">
            <div class="detail-header">
                <div class="detail-title">
                    ...
                    Repayment Installment Schedule (PDC)
                </div>
            </div>
            ...
            @if ($repaymentInstallments->isEmpty())
                <div class="alert alert-info">
                    ...
                    No repayment installment found in PDC details.
                </div>
            @else
                <div class="table-responsive mb-4">
                    <table class="table">
                        ...
                        <tbody>
                            @foreach ($repaymentInstallments as $installment)
                                <tr>
                                    <td>{{ $installment->installment_no }}</td>
                                    <td>
                                        @if (!empty($installment->cheque_date))
                                            {{ date('d/m/Y', strtotime($installment->cheque_date)) }}
                                        @else
                                            -
                                        @endif
                                    </td>
                                    <td class="fw-bold">Rs. {{ number_format($installment->amount, 2) }}</td>
            ...
            <div class="modal-content">
                ...
                <form action="{{ route('admin.repayments.store', ['user' => $user->id]) }}" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        @if ($repaymentInstallments->isNotEmpty())
                            <div class="mb-3">
                                <label class="form-label">Repayment Installment (From PDC)</label>
                                <select class="form-select" id="repaymentInstallmentSelect" name="installment_no">
                                    <option value="">Select installment</option>
                                    @foreach ($repaymentInstallments as $installment)
                                        <option value="{{ $installment->installment_no }}"
                                            data-date="{{ $installment->cheque_date }}"
                                            data-amount="{{ number_format($installment->amount, 2, '.', '') }}"
                                            data-cheque-number="{{ $installment->cheque_number }}"
                                            @if ($installment->status === 'paid') disabled @endif>
                                            Installment {{ $installment->installment_no }} -
                                            {{ !empty($installment->cheque_date) ? date('d/m/Y', strtotime($installment->cheque_date)) : '-' }}
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" class="form-control" name="amount" value="{{ old('amount') }}"
                                    step="0.01" min="0.01" max="{{ $outstandingAmount }}" required>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select" name="payment_mode" id="paymentMode" required>
                                    ...
                                    <option value="pdc" {{ old('payment_mode') === 'pdc' ? 'selected' : '' }}>PDC
                                    </option>
        ...
        $(document).ready(function() {
            ...
            if (installmentSelect) {
                installmentSelect.addEventListener('change', function() {
                    ...
                    const installmentDate = selectedOption.getAttribute('data-date');
                    const installmentAmount = selectedOption.getAttribute('data-amount');
                    const chequeNumber = selectedOption.getAttribute('data-cheque-number');

                    if (installmentDate && paymentDateInput) {
                        paymentDateInput.value = installmentDate;
                    }

                    if (installmentAmount && amountInput) {
                        amountInput.value = installmentAmount;
                    }

                    if (modeSelect) {
                        modeSelect.value = 'pdc';
                        updateReferenceField();
                    }

                    if (chequeNumber && referenceInput) {
                        referenceInput.value = chequeNumber;
                    }
                });
            }
