                    ...
                    <div class="card-body">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-4"><strong>Status:</strong>
                                {{ ucfirst(optional($user->thirdStageDocument)->status ?? 'pending') }}</div>
                            <div class="col-md-4"><strong>Submitted:</strong>
                                {{ optional($user->thirdStageDocument?->submitted_at)->format('d-m-Y H:i') ?? 'N/A' }}</div>
                        </div>
                                ...
                                <div class="col-md-6">
                                    <div class="card border-warning h-100">
                                        <div class="card-body">
                                            ...
                                            <span class="text-muted">Once resubmitted, it will return to admin
                                                review.</span>
                                        </div>
                                    </div>
                                </div>
                        ...
                        @if ($user->thirdStageDocument)
                            ...
                            @elseif($user->financial_asset_type === 'foreign_finance_assistant')
                                ...
                                <div class="row mb-3">
                                    <div class="col-md-6"><strong>Overseas Address:</strong>
                                        {{ $user->thirdStageDocument->foreign_address ?? 'N/A' }}</div>
                                    <div class="col-md-6"><strong>Foreign Contact Number:</strong>
                                        {{ $user->thirdStageDocument->foreign_contact_number ?? 'N/A' }}</div>
                                    <div class="col-md-6"><strong>SSN / Country ID:</strong>
                                        {{ $user->thirdStageDocument->foreign_ssn_or_country_id ?? 'N/A' }}</div>
                                    <div class="col-md-6"><strong>Foreign Bank Name:</strong>
                                        {{ $user->thirdStageDocument->foreign_bank_name ?? 'N/A' }}</div>
                                    <div class="col-md-6"><strong>Foreign Bank Account Number:</strong>
                                        {{ $user->thirdStageDocument->foreign_bank_account_number ?? 'N/A' }}</div>
                                </div>
                            @endif
                        @endif
                                ...
                                <ul class="list-group">
                                    ...
                                    @if (!empty($user->thirdStageDocument->domestic_paid_fees_receipt))
                                        <li class="list-group-item d-flex justify-content-between align-items-center">
                                            ...
                                            <a href="{{ asset($user->thirdStageDocument->domestic_paid_fees_receipt) }}"
                                    ...
                                    @if (!empty($user->thirdStageDocument->domestic_cancelled_cheque))
                                        <li class="list-group-item d-flex justify-content-between align-items-center">
                                            ...
                                            <a href="{{ asset($user->thirdStageDocument->domestic_cancelled_cheque) }}"
                                                target="_blank" class="btn btn-sm btn-outline-primary">View</a>
                                        </li>
                                    @endif
                                    ...
                                    @if (!empty($user->thirdStageDocument->foreign_immigration_copy))
                                        <li class="list-group-item d-flex justify-content-between align-items-center">
                                            ...
                                            <a href="{{ asset($user->thirdStageDocument->foreign_immigration_copy) }}"
                                                target="_blank" class="btn btn-sm btn-outline-primary">View</a>
                                        </li>
                                    @endif
                                    ...
                                    @if (!empty($user->thirdStageDocument->foreign_paid_fees_receipt))
                                        <li class="list-group-item d-flex justify-content-between align-items-center">
                                            ...
                                            <a href="{{ asset($user->thirdStageDocument->foreign_paid_fees_receipt) }}"
                                                target="_blank" class="btn btn-sm btn-outline-primary">View</a>
                                        </li>
                                    @endif
                        ...
                        <div class="mt-4">
                            @if ($user->thirdStageDocument && $user->thirdStageDocument->status === 'submitted')
                                <form action="{{ route('admin.third_stage_documents.approve', $user->id) }}" method="POST"
                                    class="mb-3">
                                    @csrf
                                    <div class="mb-2">
                                ...
                                <form action="{{ route('admin.third_stage_documents.sendback', $user->id) }}"
                                    method="POST">
                                    @csrf
                                    <div class="mb-2">
