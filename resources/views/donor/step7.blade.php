        ...
        <div class="row">
            ...
            <div class="col-md-6 mb-3">
                ...
                <input type="text" class="form-control"
                       value="JITO EDUCATION ASSISTANCE FOUNDATION"
                       readonly>
            </div>
            ...
            <div class="col-md-6 mb-3">
                ...
                <input type="text" class="form-control"
                       value="WATER FIELD ROAD, BANDRA (WEST)"
                       readonly>
            </div>
            ...
            <table class="table table-bordered text-center">
                ...
                <tbody>
                    @for ($i = 0; $i < $rows; $i++)
                        ...
                        <tr>
                            <td>{{ $i + 1 }}</td>
                            <td><input type="text" name="utr_no[]" class="form-control" value="{{ old('utr_no.' . $i, $entry['utr_no'] ?? '') }}"></td>
                            <td><input type="date" name="cheque_date[]" class="form-control" value="{{ old('cheque_date.' . $i, $entry['cheque_date'] ?? '') }}"></td>
                            <td><input type="number" name="amount[]" class="form-control" value="{{ old('amount.' . $i, $entry['amount'] ?? '') }}"></td>
                            <td><input type="text" name="bank_branch[]" class="form-control" value="{{ old('bank_branch.' . $i, $entry['bank_branch'] ?? '') }}"></td>
                            <td><input type="text" name="issued_by[]" class="form-control" value="{{ old('issued_by.' . $i, $entry['issued_by'] ?? '') }}"></td>
                        </tr>
                    @endfor
                </tbody>
            </table>
