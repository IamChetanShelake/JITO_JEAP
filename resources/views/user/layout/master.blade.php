    ...
    <div class="container-fluid" style="margin-top:155px;">
        <div class="row">
            ...
            @if (request()->routeIs('user.step2')) active-step @endif">
                    ...
                    @php
                        $educationalDetails = \App\Models\EducationDetail::where('user_id', auth()->id())->first();
                    @endphp

                    <li class="{{ request()->routeIs('user.step3') ? 'active' : '' }}">
                        <a href="{{ route('user.step3') }}">
                            <div
                                class="step-icon  @if ($educationalDetails && $educationalDetails->submit_status === 'submited') completed-step @endif
                                    @if (request()->routeIs('user.step2')) active-step @endif">

                                @if ($educationalDetails && $educationalDetails->submit_status === 'submited')
                                    <svg width="34" height="23" viewBox="0 0 34 23" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path d="M0 11.5L4.25 7.66667L12.75 15.3333L29.75 0L34 3.83333L12.75 23L0 11.5Z"
                                            fill="white" />
                    ...
                    @php
                        $fundingDetails = \App\Models\FundingDetail::where('user_id', auth()->id())->first();
                    @endphp
                    <li class="{{ request()->routeIs('user.step4') ? 'active' : '' }}">
                        <a href="{{ route('user.step4') }}">
                            <div
                                class="step-icon   @if ($fundingDetails && $fundingDetails->submit_status === 'submited') completed-step @endif
                                    @if (request()->routeIs('user.step2')) active-step @endif">
                                @if ($fundingDetails && $fundingDetails->submit_status === 'submited')
                                    <svg width="34" height="23" viewBox="0 0 34 23" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path d="M0 11.5L4.25 7.66667L12.75 15.3333L29.75 0L34 3.83333L12.75 23L0 11.5Z"
                                            fill="white" />
